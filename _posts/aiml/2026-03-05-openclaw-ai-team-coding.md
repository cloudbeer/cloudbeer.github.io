...
她们有自己的 GitHub 账号、有代码提交记录、有讨论帖子——不是比喻，是真实存在的账号和记录。
...
普通 AI 写代码的流程：
- 你描述需求 → AI 输出代码片段 → 你复制到编辑器 → 你手动运行 → 你把报错粘回给 AI → 循环

OpenClaw 的流程：
- 你描述需求 → AI 直接在服务器上写文件、跑命令、debug、测试、提交 → 你看结果
...
可莉 SSH 到蔻儿的机器（`10.0.1.94`），看了 systemd 状态——restart counter 已经涨到 17，进程在反复崩溃重启。继续追查，发现根本原因是：
...
**发现问题 → SSH 排查 → 定位根因 → 修复 → 文档复盘**，全程我没有介入。
